...
class Image:
    ...
    def removeBackground(self):
        ...
        thresh_img = cv2.adaptiveThreshold(
            sobel_img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 3, 0)  # 自适应二值化
        cnts, _ = cv2.findContours(
            thresh_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # 找到最大连通区域
        temp = 0
        W = 0
        H = 0
        X = 0
    ...
    def horizontal(self, img):  # 对竖直方向的黑色像素进行统计
        H, W = img.shape
    ...
    def position(self, img):
        ...
        _, threshold = cv2.threshold(
            sobel_img, 10, 255, cv2.THRESH_BINARY)  # 二值化
